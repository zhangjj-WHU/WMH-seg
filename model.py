...
class SwinTransformer_GGL(nn.Module):
    """
    Args:
...
class swintransformer_GAU_GL(SegmentationNetwork):

    def __init__(self, input_channels, num_classes,  deep_supervision=True):
    
        super(swintransformer_GAU_GL, self).__init__()
        ...
        patch_size=[2,4,4]
        window_size=4
        self.model_down=SwinTransformer_GGL(pretrain_img_size=[64,128,128],window_size=window_size,embed_dim=embed_dim,patch_size=patch_size,depths=depths,num_heads=num_heads,in_chans=input_channels)
