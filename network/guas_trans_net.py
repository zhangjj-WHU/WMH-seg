...
def GaussianMatrix(attn, sigma):
    GassMatrix = torch.zeros(attn.shape)
    num_batch = attn.shape[0]
    num_heads = attn.shape[1]
    attn = torch.clone(attn)
    for i in range(0, num_batch):
        for j in range(0, num_heads):
            attn_temp = attn[i, j]
            GassMatrix_temp = torch.zeros(attn_temp.shape)
            attn_temp = torch.clone(attn_temp)
            row = 0
            for v_i in attn_temp:
                col = 0
                for v_j in attn_temp:
                    GassMatrix_temp[row, col] = Gaussian(v_i.T, v_j.T, sigma)
                    col += 1
                row += 1
                GassMatrix[i, j] = GassMatrix_temp
    return GassMatrix


def GaussianMatrix_2(attn, sigma):
    # attn: [B, H, N, D]  (batch, head, tokens, dim)
    B, H, N, D = attn.shape

    # 计算 pairwise squared Euclidean distance in a vectorized manner
    attn_flat = attn.view(B * H, N, D)  # [B*H, N, D]

    # 扩展并计算 ||x - y||^2 = ||x||^2 + ||y||^2 - 2x·y
    x_square = (attn_flat ** 2).sum(dim=-1, keepdim=True)  # [B*H, N, 1]
    y_square = x_square.transpose(1, 2)  # [B*H, 1, N]
    xy = attn_flat @ attn_flat.transpose(1, 2)  # [B*H, N, N]
    dist_squared = x_square + y_square - 2 * xy  # [B*H, N, N]

    safe_sigma = max(sigma, 1e-7)
    dist_squared = dist_squared.clamp(min=0)

    # 计算 Gaussian 相似度
    gaussian_sim = torch.exp(-dist_squared / (2 * safe_sigma ** 2))  # [B*H, N, N]

    return gaussian_sim.view(B, H, N, N)

class LearnableGaussianMatrix(nn.Module):
    def __init__(self, init_sigma=1.0):
        super(LearnableGaussianMatrix, self).__init__()
        # 用 log(sigma) 初始化，使得 sigma 保持正值
        self.log_sigma = nn.Parameter(torch.log(torch.tensor(init_sigma)))

    def forward(self, attn):  # attn: [B, H, N, D]
        B, H, N, D = attn.shape
        attn_flat = attn.view(B * H, N, D)
        ...
        sigma = torch.exp(self.log_sigma)  # 确保 sigma > 0
        gaussian_sim = torch.exp(-dist_squared / (2 * sigma ** 2))

        return gaussian_sim.view(B, H, N, N)


class LearnableGaussianAttention_2(nn.Module):
    ...
    def forward(self, attn):  # attn: [B, H, N, D]
        B, H, N, D = attn.shape
        attn_flat = attn.view(B * H, N, D)
        attn_flat = F.normalize(attn_flat, dim=-1)

        x2 = (attn_flat ** 2).sum(dim=-1, keepdim=True)
        y2 = x2.transpose(1, 2)
        xy = attn_flat @ attn_flat.transpose(1, 2)
        dist_squared = x2 + y2 - 2 * xy
        del xy, attn_flat, x2, y2, attn
        dist_squared = torch.clamp(dist_squared, min=1e-6, max=1e2)

        sigma = F.softplus(self.log_sigma) + 1e-6
        sim = torch.exp(-dist_squared / (2 * sigma ** 2))
        sim = F.softmax(sim, dim=-1)  # 数值稳定
        sim = sim.view(B, H, N, N)
        return sim

def Gaussian(x, z, sigma):
    return torch.exp((-(torch.norm(x - z) ** 2)) / (2 * sigma ** 2))
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        ###################
        g_layer = LearnableGaussianAttention_2(init_sigma=0.5)
        g = g_layer(attn)  # [B, H, N, N]
        attn = g+torch.matmul(g, attn)
        # print(attn)
        ###################

        ###################
        # g_layer = LearnableGaussianMatrix(init_sigma=0.5)
        # g = g_layer(attn)
        # attn = g * attn
        ###################

        # print(attn.shape)  # (batch,head,N,N)
        # Gaussian weight
        # g = GaussianMatrix_2(attn, 0.4)
        # attn = g @ attn
        # print(attn.shape)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        # gaussian weight
        # attn_x, attn_y, attn_z,mixed_value = self.attn(x_windows, mask=attn_mask)
        # gaussian_input = (attn_windows,attn_x,attn_y,attn_z,mixed_value )
        # attn_windows = self.gaussiantrans(gaussian_input)
        ...
        x_pe = self.mlp(self.norm2(FPE(x)))
        x = x.view(B, S * H * W, C)
        x_pe = x_pe.view(B, S * H * W, C)

        # FFN
        x = shortcut + self.drop_path(x)
        # x = x + self.drop_path(self.mlp(self.norm2(FPE(x))))
        x = x + self.drop_path(x_pe)
...
class GaussianTrans(nn.Module):
    ...
    def forward(self, x):
        x, atten_x_full, atten_y_full, atten_z_full, value_full = x  # x(b, h, w, c) atten_x_full(b, h, w, w)   atten_y_full(b, w, h, h) value_full(b, h, w, c)
...
class SwinTransformer(nn.Module):
    ...
    def inflate_weights(self):
        ...
        the weight of swin2d nnFormer_trained_models should be inflated to fit in the shapes of
        the 3d counterpart.
...
class swintransformer(SegmentationNetwork):
    ...
    def __init__(self, input_channels, num_classes, deep_supervision=True):
        ...
        patch_size = [2, 4, 4]
        window_size = 4
        self.model_down = SwinTransformer(pretrain_img_size=[64, 128, 128], window_size=window_size,
        ...
        self.encoder = encoder(pretrain_img_size=[64, 128, 128], embed_dim=embed_dim, window_size=window_size,
